...
//!
//! Tenancy (ADR-0029): the module carries none, but the GL envelope and outbox record are sourced
//! fail-closed from the ambient org request scope, so each movement runs inside
//! `with_org_request_scope` — the test stands in for the composing service.
...
async fn egc1_issue_splits_par_and_premium() {
    ...
    let out = with_org_request_scope(
        &pool,
        scope.clone(),
        svc.issue_shares(
            IssueShares {
    ...
    assert_eq!(
        gl.account_balance(a.share_capital),
        dec("-100000"),
        "capital at par (credit)"
    );
    assert_eq!(
        gl.account_balance(a.share_premium),
        dec("-50000"),
        "premium = excess over par (credit)"
    );
...
async fn egc2_issue_at_par_has_no_premium_line() {
    ...
    assert_eq!(
        gl.last().lines.len(),
        2,
        "at par: Dr Bank · Cr Capital, no premium"
    );
...
async fn egc3_transfer_moves_ownership_no_gl() {
    ...
    let over = with_org_request_scope(
        &pool,
        scope.clone(),
        svc.transfer_shares(
            TransferShares {
                share_class_id: class,
                from_shareholder_id: bob,
                to_shareholder_id: alice,
                quantity: dec("31"),
                txn_date: today(),
            },
            &CapturingSink::new(),
    ...
    .unwrap();
    assert!(
        matches!(over, Err(EquityError::InsufficientShares { .. })),
        "Bob holds only 30"
    );
}
...
async fn egc5_declare_then_pay_dividend() {
    ...
    assert_eq!(
        gl.account_balance(a.dividend_payable),
        dec("-5000"),
        "payable booked (credit)"
    );

    with_org_request_scope(
        &pool,
        scope.clone(),
        svc.pay_dividend(div.id, a.bank, today(), &gl, &CapturingSink::new()),
    )
    .await
    .unwrap()
    .unwrap();
...
async fn egc6_holdings_and_dividend_allocations() {
    ...
    let holdings = svc.holdings(class).await.unwrap();
    ...
    assert_eq!(
        svc.class_shares_outstanding(class).await.unwrap(),
        dec("100")
    );
    ...
    assert_eq!(
        total, div.amount,
        "Σ per-holder allocations == the declared total"
    );
    assert_eq!(
        allocs
            .iter()
            .find(|x| x.shareholder_id == alice)
            .unwrap()
            .amount,
        dec("3500")
    );
