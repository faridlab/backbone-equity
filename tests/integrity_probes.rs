...
//!
//! Tenancy (ADR-0029): the module carries none, but the outbox record is sourced fail-closed from the
//! ambient org request scope, so writes (and the outbox read-back) run inside `with_org_request_scope`
//! — the test stands in for the composing service.
...
async fn eip1_issue_below_par_refused() {
    let pool = pool().await;
    let (_company, _scope, svc, a, class, holder) = setup(&pool).await;
    let r = svc
        .issue_shares(
            IssueShares {
        ...
        .await;
    assert!(
        matches!(r, Err(EquityError::Invalid(_))),
        "below-par issue refused"
    );
}
...
// Both branches run under one request scope (one composing-service request); the advisory lock is taken on
// each branch's own transaction, so the race survives the shared scope.
#[tokio::test]
async fn eip2_concurrent_removal_cannot_go_negative() {
    ...
    let (r1, r2) =
        with_org_request_scope(&pool, scope.clone(), async { tokio::join!(bb(s1), bb(s2)) })
            .await
            .unwrap();
...
async fn eip3_buyback_with_no_holding_refused() {
    ...
    let r = svc
        ...
        .await;
    assert!(
        matches!(r, Err(EquityError::InsufficientShares { .. })),
        "cannot buy back from an empty position"
    );
}

// EIP-4 — the lifecycle event is durable: with the in-proc publish dropped, DividendDeclared is still staged
// in the outbox for the relay. The read-back rides the request-dedicated connection inside the same request
// scope the producer wrote under — the outbox table is relay infrastructure and stays fenced.
#[tokio::test]
async fn eip4_lifecycle_event_durable() {
    ...
    let staged = with_org_request_scope(&pool, scope.clone(), async {
        let div = svc.declare_dividend(DeclareDividend {
            share_class_id: class, per_share_amount: dec("50"), declaration_date: today(),
        ...
        fetch_optional_row_scoped(
            &pool,
            sqlx::query(
                "SELECT 1 FROM equity.outbox_events WHERE aggregate_id=$1 AND event_type='DividendDeclared'")
                .bind(div.id.to_string()),
        ).await.unwrap().is_some()
    }).await.unwrap();
    assert!(
        staged,
        "DividendDeclared durably staged despite the dropped publish"
    );
}
...
async fn eip5_negative_quantity_rejected_at_db() {
    let pool = pool().await;
    let (_company, _scope, _svc, _a, class, holder) = setup(&pool).await;
    // A raw INSERT — the trust boundary the generic CRUD create/update endpoints sit on.
    let bad = sqlx::query(
        r#"INSERT INTO equity.share_transactions
             (id, share_class_id, shareholder_id, txn_type, quantity, price_per_share, amount, txn_date, gl_posted)
           VALUES (gen_random_uuid(), $1, $2, 'buyback'::share_txn_type, -5, 0, 0, DATE '2026-01-01', false)"#,
    )
    .bind(class).bind(holder)
    .execute(&pool).await;
    assert!(
        bad.is_err(),
        "a negative-quantity register row cannot be inserted through any writer"
    );
}
