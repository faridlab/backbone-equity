//! Domain policy for ShareTransaction aggregate
//!
...
/// Domain policy for ShareTransaction — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<ShareTransaction>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type ShareTransactionDomainPolicy = PermitAllPolicy<ShareTransaction>;
